...
This is a Streamlit-based application that allows users to convert any blog post into a podcast. The app uses GROQ's free models for summarization, Firecrawl for scraping blog content, and ElevenLabs API for generating audio. Users simply input a blog URL, and the app will generate a podcast episode based on the blog.
...
- **API Key Integration**: Requires GROQ, Firecrawl, and ElevenLabs API keys to function, entered securely via the sidebar.
...
1. **API Keys**:
    - **GROQ API Key**: Sign up at Groq to obtain your API key.
...
1. Clone this repository:
   ```bash
   git clone https://github.com/vaibhavfegade/LLMProjects
   cd agents
   ```
...
1. Start the Streamlit app:
   ```bash
   streamlit run ai_blog_to_podcost_agent.py
   ```
